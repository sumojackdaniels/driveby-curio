// Pre-generate mp3 audio for walking tour waypoints using ElevenLabs.
//
// Usage:
//   ELEVENLABS_API_KEY=$(gcloud secrets versions access latest --secret=ELEVENLABS_API_KEY) \
//     npx tsx src/scripts/synthesize-walking-tours.ts
//
// Output: ../DriveByCurio/WalkingTours/{tourId}/{waypointId}/content.mp3
//         ../DriveByCurio/WalkingTours/{tourId}/{waypointId}/nav.mp3
//
// These are bundled into the iOS app as resources.
...
const OUT_ROOT = path.join(__dirname, "..", "..", "..", "DriveByCurio", "WalkingTours");

// Walking tour content — mirrors AuthoredWalkingTours.swift
interface WaypointContent {
  id: string;
  narrationText: string;
  navInstructionText?: string;
}
...
const tours: TourContent[] = [
  {
    id: "huntington-bradmoor",
    waypoints: [
      {
        id: "hb-01-starting-point",
        narrationText: `You're standing at the corner of Huntington Parkway and Bradmoor Drive, in the heart of what was, until about 1948, farmland and small estates. Everything you see around you — every house, every sidewalk, every mature oak — exists because of a specific moment in American history.
...
If you look carefully, you can spot the original homes versus the renovated ones. The originals are modest — maybe 1,200 square feet, single-story or a story and a half with dormers. They sit low on their lots. The renovations and teardown rebuilds are dramatically larger, often pushing the setback limits. That contrast tells the story of how Bethesda changed from an affordable suburb to one of the most expensive zip codes in America.`,
      ...
      {
        id: "hb-03-bradmoor",
        narrationText: `Welcome to Bradmoor Drive. Notice how the name sounds — Bradmoor. It's meant to evoke the English countryside, rolling moors, something genteel and landed. Mid-century subdivision developers were masters of aspirational naming. The lots were a quarter acre, not a country estate, but the name did its work.
...
Both styles shared a new relationship with the car. Look at the garages and driveways. In Huntington Terrace, garages were often detached, almost an afterthought. Here on Bradmoor, the garage is integrated into the house — sometimes it's the most prominent feature of the facade. The car had gone from luxury to necessity in a single decade, and the house adapted.`,
      ...
      },
      {
        ...
        narrationText: `You can't see it from here, but less than two miles west of us is the Burning Tree Club — one of the most exclusive private golf clubs in America. It's been men-only since it opened in 1922, and its membership has included presidents Eisenhower, Nixon, Ford, and George H.W. Bush. The name comes from a Native American practice of setting fire to hollow trees to smoke out game.
...
Thank you for walking with me. I hope you'll look at these quiet streets a little differently now.`,
  ...
  },
  {
    ...
    waypoints: [
      {
        id: "mc-01-entrance",
        narrationText: `Welcome to McCrillis Gardens, five acres of shade garden tucked into a residential neighborhood in Bethesda. Most people who live within a mile of here don't know it exists. That's part of its charm.
...
This means the garden speaks a different visual language than what you might be used to. Instead of big, showy blooms, you'll see subtle variations in leaf texture, shape, and color. Shade gardening is an art of nuance. Let me show you what I mean.`,
      ...
      {
        id: "mc-02-azaleas",
        narrationText: `McCrillis Gardens is renowned for its azalea collection — hundreds of plants representing dozens of varieties, both native and hybrid. In late April and early May, this area erupts in color: whites, pinks, corals, magentas, and a deep red that's almost purple.
...
William McCrillis collected both kinds obsessively. The Piedmont soil here — slightly acidic, well-drained but moisture-retentive — is perfect for azaleas. The same geology that made this area difficult farmland makes it ideal azalea territory. Sometimes the land knows what it wants to grow.`,
      ...
      },
      {
        ...
        narrationText: `Look up. The large trees overhead aren't part of the McCrillis garden — they predate it by decades, maybe a century. These are remnant canopy trees from the forest that covered this part of Montgomery County before it was cleared for agriculture, and then for suburbs.
...
This is why Montgomery County's tree protection ordinances matter so much. Each one of these old trees is an ecological engine that takes a century to build and an afternoon to destroy.`,
      ...
      },
      {
        ...
        narrationText: `This section is where shade gardening reveals itself as an art form. Look at the ground layer around you: hostas with leaves the size of dinner plates next to delicate maidenhair ferns. Bold-textured rodgersias beside lacy astilbe. Every planting is a composition in leaf shape, texture, and shade of green.
...
That's the philosophy of this entire garden, really. It rewards attention. It's not trying to impress you from a distance. It's asking you to slow down, look closely, and notice the details.`,
      ...
      {
        id: "mc-05-stream",
        narrationText: `Every garden has a low point where water naturally collects, and in a well-designed garden, that's not a problem — it's an opportunity. This is McCrillis's stream garden, where moisture-loving plants thrive in the damp soil near the drainage channel.
...
The McCrillis garden doesn't have a dramatic fountain or waterfall. It has what the land provides: a low spot where rainwater collects and moves. That restraint is part of what makes it feel authentic rather than constructed.`,
      ...
      {
        id: "mc-06-seasonal",
        narrationText: `Before you leave, I want to share the most important idea in garden design — the one that separates a good garden from a great one. A great garden is designed for twelve months, not three.
    ...
    waypoints: [
      {
        ...
        narrationText: `You're standing at Boundary Bridge, and the name is literal — the state line between Maryland and the District of Columbia runs right through here. Step ten feet south and you're in a national park. Step ten feet north and you're in Montgomery County.
...
Notice the rock outcrops along the creek banks. We're approaching something geologically significant — a place where the earth changes underneath you. More on that at the next stop.`,
      ...
      {
        id: "rc-03-fall-zone",
        narrationText: `Right about here, you're standing on one of the most important geologic boundaries on the East Coast. It's called the fall zone — or the fall line — and it's where the hard, ancient rock of the Piedmont Plateau meets the soft, young sediment of the Atlantic Coastal Plain.
...
The geology also explains why Rock Creek cut such a deep valley. The creek is working through that hard Piedmont rock, which resists erosion — so instead of spreading out into a wide, gentle floodplain, the water carved a narrow gorge. That's why it feels like a mountain hollow in here, even though you're five miles from the White House.`,
      ...
      },
      {
        ...
        narrationText: `This is Rolling Meadow Bridge, and it's a small masterpiece of a style called parkitecture — architecture designed to disappear into the landscape. Look at how it's built: concrete and local stone, with proportions that echo the boulders in the creek. The handrails were originally wood. Nothing about it screams "engineered structure." It looks like it could have grown here.
...
Take a moment on the bridge. Look upstream and downstream. The creek is framed perfectly from here — that's not an accident. These bridges were placed at scenic viewpoints. The engineers were designing experiences, not just infrastructure.`,
      ...
      {
        id: "rc-05-deep-woods",
        narrationText: `Stop here and just listen for a moment.
...
This is our turnaround point. We'll head back north along the same route, but I have one more thing to show you.`,
      ...
      {
        id: "rc-06-return",
        narrationText: `As you walk these last few hundred yards back to Boundary Bridge, I want to leave you with something to think about.
...
async function main() {
  ...
  for (const tour of tours) {
    ...
    for (const wp of tour.waypoints) {
      const wpDir = path.join(OUT_ROOT, tour.id, wp.id);
      fs.mkdirSync(wpDir, { recursive: true });

      // Content audio
      const contentPath = path.join(wpDir, "content.mp3");
      if (!FORCE && fs.existsSync(contentPath)) {
        console.log(`  - ${wp.id}/content: skip (exists)`);
        totalSkipped++;
      } else {
        process.stdout.write(`  - ${wp.id}/content: synthesizing... `);
        try {
          const buf = await synthesize(wp.narrationText);
          fs.writeFileSync(contentPath, buf);
      ...
      // Nav instruction audio
      if (wp.navInstructionText) {
        const navPath = path.join(wpDir, "nav.mp3");
        if (!FORCE && fs.existsSync(navPath)) {
          console.log(`  - ${wp.id}/nav: skip (exists)`);
          totalSkipped++;
        } else {
          process.stdout.write(`  - ${wp.id}/nav: synthesizing... `);
          try {
            const buf = await synthesize(wp.navInstructionText);
            fs.writeFileSync(navPath, buf);
            console.log(`✓ (${(buf.length / 1024).toFixed(1)} KB)`);
            totalGenerated++;
          } catch (err) {
            console.log(`✗`);
